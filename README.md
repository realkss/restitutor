...
- `extension/` — the stage-2 browser extension (Manifest V3): a content script that finds math whose TeX the page carries and translates it on click in an in-page panel, with the extraction provenance shown. `extension/src/page.ts` reads the page — spans, the miner's surface, the page's readings — against the standard DOM only, so the same code runs in the browser and in node. An equation group's rows (LaTeXML renders each cell of an eqnarray as its own math element) are rejoined into statements, and a line holding several statements is split, before anything reaches the engine (`src/tex.ts`).
...
- `scripts/` — the builds, the table generator, the store packager, `check-site-sync.mjs` (`npm run sync-check`), which verifies the site's vendored engine is identical to `src/` up to line endings and BOM, and `ledger.ts`, the decline ledger: it runs the page reading over a corpus of captured pages and names every outcome by the engine's own reason. `docs/decline-ledger.md` is its output over 18 pages, and product-design §10 reads it: why the tool says no, and what that decides. `recall.ts` scores convention detection on the same papers against adjudicated ground truth (`docs/data/detection-truth.json`, in this project's words); `docs/detection-recall.md` is its output. This repository is the source of record.
