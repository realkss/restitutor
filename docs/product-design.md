...
src/unitsEngine.ts       — the engine: KaTeX-AST reading, the dimensional solve, the decline
                           ...
                           Vendored: the site carries it byte-identical (scripts/check-site-sync.mjs);
                           changes land in verified batches and are re-synced.
...
## 10. The decline ledger (2026-09-11)

`scripts/ledger.ts` runs the extension's page reading over a corpus of captured pages and names every outcome by the engine's own reason (`src/ledger.ts`); `docs/decline-ledger.md` is its output over 19 pages (17 ar5iv papers cited by the census, the two Wikipedia fixtures; the ar5iv captures stay out of the repository) — 3,194 pooled equations translated to SI under each page's own registry. It was built to answer one question with data: would a context reader (retrieval, or a model) earn its keep? The reading:

- **Translated or already in target form: 4%.** The GR profile on a corpus that is mostly not GR.
- **Unknown symbol: 31%** of equations. Of the 2,324 symbol-occurrences behind them, **12%** are of a symbol the same page mentions under a declaration cue. That is the whole territory a context reader could claim: at most one equation in twenty-five, and the top of the list is not prose at all — `∂` (141 equations), `δ`, `D`, `O`, operators the engine's reader does not handle — followed by single letters no sentence pins down and by string-theory symbols (`X^μ`, `α'`) that only a profile can bind.
- **Unsupported construct: 29%.** Integrals and sums with a measure (175), sub/superscript shapes the reader cannot read (141), primed symbols (100), relations the engine does not handle (91), exponents it cannot read (156), comma-derivative indices (58). All of it is the engine's reader, none of it is reading the page.
- **Fragment: 13%.** The carrier was not one equation — an alignment block, a list of statements, a row cut at its relation. Extraction can split those into rows before the engine sees them.
- **Unparseable TeX: 6%** (LaTeXML macros KaTeX does not know); no anchor 11%; refused, reassembly, no completion, k_B kept explicit: 2% each or less.

So the AI layer stays parked, on evidence: its ceiling is about 4% of equations and it would sit behind three larger, fully deterministic gains — splitting fragments (extraction), teaching the reader the constructs it declines (the frozen engine, released deliberately and synced to the site), and a second profile for the symbols no prose defines. Detection on the same corpus is its own finding: 7 of the 19 pages returned no convention evidence at all, on papers that state their units in a sentence the chain matcher should catch; that recall gap is the next measurement to take, with the same script.

**Fragments, after the fact (2026-09-11).** The fragments were not what they looked like. LaTeXML renders an eqnarray as a table whose cells are separate math elements — "s² = …" | "=" | "…" on one row, "=" | "…" on the next — so the carriers the engine refused were the cells, and the equations were whole in the markup. The extractor now rejoins the cells of a row and folds a group's rows into statements (`src/tex.ts foldRows`: a row that continues the previous one is joined to it, a row that begins with a relation restates the previous left side), splits a line into its statements at "and", a semicolon, a \qquad, or a spaced comma when every piece is a relation (`splitStatements`), and reads an aligned environment the same way; the panel decorates every row a statement spans and prints one result per statement. Rerun on the same corpus (3,276 statements): fragments 13% → 7%, and the "no anchor" class fell from 353 to 18, since most of those were lone cells. What the rejoined rows became is the honest part of the result: unsupported constructs rose from 29% to 45% and unparseable TeX from 6% to 11%, because a whole equation from Carroll's notes carries `^{\rm T}`, a primed coordinate, a `\matrix`, or an index pair the reader cannot read. The fragment class was hiding engine work, not saving it. The remaining 7% is mostly the engine reading an argument comma as a list separator (`X^μ(σ,τ) = …`), which is also the reader's, plus tuples and index ranges that are not equations.

**A correction to the count (2026-09-23).** The 19 pages held one paper twice: the string-compactification guide in its ar5iv and its arXiv HTML rendering, which carry different carriers and so passed the ledger's exact-prefix duplicate check. The ledger now compares pages by the overlap of their non-trivial equations (renderings of one paper share 74–100%, distinct pages at most 17%). Rerun: 18 pages (16 papers, the two Wikipedia fixtures), 3,153 statements — unsupported 47%, unknown symbol 24%, unparseable 9%, fragment 7%, and 8% of the blocking symbol-occurrences cue-mentioned. The reading stands.

**Detection recall (2026-09-23).** The ledger's detection finding above was a question, and it is now measured. One reader per paper (workflow `wf_d1a4b365-a71`) read each of the 16 papers and named the census conventions its equations are consistent with and any sentence that declares one; every verdict was adjudicated. The truth is `docs/data/detection-truth.json`, in this project's words (no text from the papers), and `scripts/recall.ts` rescores any corpus against it; `docs/detection-recall.md` is its output. The silent pages were mostly right to be silent: a paper written in unstated natural units gives no evidence, and census §6 forbids reading absence as evidence. The failures were elsewhere. Two pages were narrowed to sets that excluded the truth — a normalized weight μ^c = 1 in the HAYSTAC analysis read as c = 1, and CODATA 2022's mention of atomic units, which describes a cited calculation on pages that print ħ throughout — and four papers stated their convention in a sentence the detector did not act on (a fifth reader's claim, "a.u." tags as Hartree units, was rejected). Now: the overall set contains every true convention on 13 papers (8 narrowed, 5 silent), stays silent on the one to which none applies, holds 5 of 8 on the review that switches between cosmology and numerical relativity by section, and reports a conflict with both sides on the LAMMPS tutorial, which declares Lennard-Jones units and prints k_B T. No page excludes its truth; no page with nothing to find is narrowed; all four missed declarations are acted on. What changed, in `src/detect.ts` and the page reading:

- Chains read `\equiv` and Newton's constant with its subscript (M_pl⁻² ≡ 8πG_N = 1). A lone c = 1, e = 1 or G = 1 counts only in a clause that speaks of units: a central charge c = 1 and a + b + c = 1 are not declarations.
- A display equation ends the sentence. The page reading marks the boundary between a paragraph's prose and its blocks, so the prose before an equation and the prose after it no longer read as one sentence to the frame tests, and the anti-frame looks at the declaration's own clause: a comparison two clauses later no longer vetoes it.
- Declarations in words: constants named and set to unity, a constant said to be absorbed, temperature measured in energy. Captions are read, since AstroGK declares k_B = 1 only in the caption of its symbol table.
- A named system or a code is weighed against the constants the page prints strongly: where every convention it implies absorbs a constant the page writes out, the declaration is shown as contradicted instead of intersected.
- A hypothetical (what a quantity would be if one worked in SI) does not declare.
- A code declares only when the prose around it, not its own name, speaks of the page's computation; a catalog waveform's provenance and the infrastructure a code runs on no longer count (the LIGO–Virgo guide is now rightly silent).

Left for later, each a precision risk until a page earns it: constants in their usual company (c in mc² and ħc, e in eħ/2m, h in hν) as stronger printed evidence; α′, κ and M_pl as carriers of G; the Bohr magneton and the cyclotron frequency as fingerprints; the Heaviside–Lorentz rider that absorbs 1/√4π; "in units of the central mass" as M = 1; per-span reporting where a page's declared units and printed constants disagree by section (LAMMPS). Two readings were tried and rejected: "a.u." as atomic units (it is also arbitrary units) and "geometric units" as a declaration (Hogg names it as other authors' convention).

**The engine's reader (2026-09-24/25).** The ledger named the engine's reader as the largest deterministic gain, and the owner opened the vendored file. Seven construct families were designed and attacked one by one (workflow `wf_1ff62555-987`) and integrated into a twenty-step plan; steps 0–18 are in. Each step was gated by the full suite, a before/after snapshot of every statement's outcome (the ledger's statements at SI, every site display at all six floater targets) and a refute-first verifier. Measured on the ledger's 3,153 statements: translated or already in target form 128 → 253 (4% → 8%); unsupported constructs 47% → 38%; fragments 7% → 2%; reassembly faults 64 → 2; unknown symbols 24% → 35%. The last number is the finding the 09-11 reading predicted: once a construct is read, the next wall on a corpus that is mostly not GR is the dictionary. On the site, every equation that translated before gives the same output at all six targets, two Conventions lists now translate, and about half the site's declines now name the real blocker.

The larger gain is correctness. The engine had been shipping wrong outputs that these steps remove: two statements read as one product (`t = 0 \qquad r = 2M` gave t = 0·r/c), an upright unit read as a product (2π Hz as H·z), a value of a constant restored (c = −26 gave −26c), `\neq` dead since KaTeX 0.16.47, a constant set after an unparenthesized function argument (tanh φ·c), numerals fused into a new number by a stripped constant. Every new reading rests on a notational fact; where the notation does not settle a reading the engine declines and says why: an accent over c or G, a summation index used after its sum's term, Dirac notation, a superscript beside derivative indices, a bare 1 compared with a dimensional quantity in a list. Two lessons for the process. A guard that matches TeX shapes loses to the next bracket; the rules that held decide on the analysis (an accent is judged by what its body analyzes to; one net compares the identities of numerals between the output and the verbatim re-emission). And verification needs a calibration: a blocker is an input an author could plausibly write, or a guarantee the code claims and breaks; constructions that only defeat a check are recorded, not blocked on.

Still the owner's (plan step 19, each a ruling on data, not code): what a coordinate name in a lower index carries (∂_t as s⁻¹, or the 2026-08-17 index ruling with angular components declined); `;` and `,` as ∇ and ∂ in GR index lists (the code path ships with no GR data); `\hat` and `\tilde` as symbols of their own rather than read through; what a bare "= 1" side means outside a list; λ in the measure position; `\lesssim`; per-page scope for the constants (the central charge c on Tong's notes is still read as the speed of light); and registry readings on the Hawking and Teukolsky pages, Kruskal v, the FRW scale factor a and a generic T.
