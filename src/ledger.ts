...
const STRUCTURAL: [RegExp, OutcomeClass][] = [
  ...
  [
    /not supported|could not be read|could not read|cannot read|floating super\/subscript|symbolic exponent|unsupported (?:relation|accent|differential)|closing delimiter|cannot pair|whose pairing is ambiguous|Dirac bra–ket notation|a modulus or a determinant|in this position|time derivative of a compound|integrals, sums, and limits|an angular coordinate index|the upright (?:word|words|letter) |the multi-letter name |an unspecified constant|a constant to restore inside the font|the arrow “|the exchange “|a colon that is not part of|which the engine does not read|neither a power nor a dictionary index|a sign directly beside|a component index or a power|, not the constant [cG]$/,
    "unsupported",
  ],
]
