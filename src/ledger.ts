...
const STRUCTURAL: [RegExp, OutcomeClass][] = [
  ...
  [
    /not supported|could not be read|could not read|cannot read|floating super\/subscript|which may be an exponent or an index|unsupported (?:relation|accent|differential)|closing delimiter|cannot pair|whose pairing is ambiguous|Dirac bra–ket notation|a modulus or a determinant|in this position|time derivative of a compound|an angular coordinate index|the upright (?:word|words|letter) |the multi-letter name |an unspecified constant|a constant to restore inside the font|the arrow “|the exchange “|a colon that is not part of|which the engine does not read|neither a power nor a dictionary index|a sign directly beside|a component index or a power|, not the constant [cG]$|mixes a label with indices|a label or mark on the constant|a dot on the indexed symbol|the operator “.*” with nothing to act on|an upright “d” with nothing after it|a differential of symbolic order|a detached superscript “|with no measure \(d…\) of its own|a functional integral|with its measure first and more terms|which the engine does not restore|as the name of a set|cannot be paired with one variable|an integral over “|the vector measure|a function whose argument runs into|of a dimensional argument|whose undelimited argument|does not pass through|which the engine cannot name|which the engine does not re-emit/,
    "unsupported",
  ],
]
