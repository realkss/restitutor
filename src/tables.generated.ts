// GENERATED by scripts/gen-tables.mjs from docs/data/fingerprints.json,
// docs/data/glossary.json, docs/data/codes.json and docs/data/forks.json.
// Edit those files, then `npm run gen:tables`.
...
export const FORK_RULES: ForkRule[] = [
  ...
  },
  {
    ...
    "surface": "equation",
    "pattern": "(?:S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\^\\s*\\{?\\s*\\(\\s*(?:2|II)\\s*\\)\\s*\\}?\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)|(?<![0-9.)}\\]]\\s{0,3})\\\\int(?:\\\\limits)?\\s*_\\s*\\{?\\s*-\\s*\\\\infty\\s*\\}?\\s*\\^\\s*\\{?\\s*\\+?\\s*\\\\infty\\s*\\}?(?=[^;]{0,200}?(?:S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)[\\s\\S]{0,160}?d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])|d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])[\\s\\S]{0,160}?S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu))))",
  ...
  {
    "id": "psd-one-sided-integral",
    "fork": "PSD one- vs two-sided; per-Hz vs per-(rad/s)",
    "branch": "one-sided (single-sided) spectral density: all the power sits on 0 ≤ f < ∞, so S₁(f) = 2S₂(f) — the inner product prints 2∫₋∞^∞ or 4Re∫₀^∞, and ⟨x²⟩ = ∫₀^∞ S(f) df",
    "rival": "two-sided (double-sided) spectral density integrated over −∞ < f < ∞ with no prefactor, S₂ = S₁/2",
    "magnitude": "×2 on S(f) (and on ρ² / SNR² built from it): a one-sided S_n read as two-sided halves every SNR² and doubles every quoted noise floor; a further 2π between per-Hz and per-(rad/s)",
    "surface": "equation",
    "pattern": "(?:(?<!\\^\\s*\\{?\\s*\\(\\s*(?:2|II)\\s*\\)[\\s\\S]*)(?![\\s\\S]*\\^\\s*\\{?\\s*\\(\\s*(?:2|II)\\s*\\))S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\^\\s*\\{?\\s*\\(\\s*(?:1|I)\\s*\\)\\s*\\}?\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)|(?<![0-9.^_A-Za-z}\\]])2\\s*(?:\\\\Re|\\{\\s*\\\\rm\\s*Re\\s*\\})?\\s*\\\\int(?:\\\\limits)?\\s*_\\s*\\{?\\s*-\\s*\\\\infty\\s*\\}?\\s*\\^\\s*\\{?\\s*\\+?\\s*\\\\infty\\s*\\}?(?=[^;]{0,200}?(?:/\\s*\\(?\\s*S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)[\\s\\S]{0,160}?d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])|d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])[\\s\\S]{0,160}?/\\s*\\(?\\s*S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)))|(?:S|G|P)\\s*(?:_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}))?\\s*\\(\\s*(?:f|\\\\omega|\\\\nu)\\s*\\)\\s*=\\s*2\\s*\\\\int(?:\\\\limits)?\\s*_\\s*\\{?\\s*-\\s*\\\\infty\\s*\\}?\\s*\\^\\s*\\{?\\s*\\+?\\s*\\\\infty\\s*\\}?[^;=]{0,160}?d\\s*\\\\tau(?![A-Za-z])|\\^\\s*\\{?\\s*2\\s*\\}?[^=;]{0,14}=\\s*\\\\int(?:\\\\limits)?\\s*_\\s*\\{?\\s*0\\s*\\}?\\s*\\^\\s*\\{?\\s*\\+?\\s*\\\\infty\\s*\\}?\\s*S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)\\s*\\|?\\s*\\)\\s*[^=;]{0,20}?d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])|(?<![0-9.^_A-Za-z}\\]])4\\s*(?:\\\\Re|\\{\\s*\\\\rm\\s*Re\\s*\\})?\\s*\\\\int(?:\\\\limits)?\\s*_\\s*\\{?\\s*0\\s*\\}?\\s*\\^\\s*\\{?\\s*\\+?\\s*\\\\infty\\s*\\}?(?=[^;]{0,200}?(?:/\\s*\\(?\\s*S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu)[\\s\\S]{0,160}?d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])|d\\s*(?:f|\\\\omega|\\\\nu)(?![A-Za-z])[\\s\\S]{0,160}?/\\s*\\(?\\s*S\\s*_\\s*(?:\\{[^{}=]{0,14}\\}|\\\\?[A-Za-z0-9]{1,3}|(?=\\s*\\())\\s*\\(\\s*\\|?\\s*(?:f|\\\\omega|\\\\nu))))",
    "tex": "\\rho^{2} = 4\\int_{0}^{\\infty}\\frac{|\\tilde h(f)|^{2}}{S_n(f)}\\,df = 2\\int_{-\\infty}^{\\infty}\\frac{|\\tilde h(f)|^{2}}{S_n(|f|)}\\,df",
    "meaning": "A prefactor 2 on the full-line noise-weighted inner product or on the lag integral that defines the density, a prefactor 4 on the positive-frequency inner product, a mean-square identity whose density integral runs 0→∞, or an S^{(1)}(f) tag: the density is one-sided — half the frequency line carries all the power, so S₁ = 2S₂."
  }
]
